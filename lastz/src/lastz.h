...
typedef struct control
	...
	int			selfCompare;	// (I) true  => seq1 and seq2 are the same
								//              .. sequence
	...
	score		ambiMismatch;	//     (non-negative) penalty for mismatches
								//     .. among ambiguous DNA
	int			hspToGapped;	//     true => extend HSPs to gapped alignments
	...
	u32			searchLimit;	//     maximum number of "HSPs" allowed for a
								...
								//     .. alignments if hspToGapped is true
	...
	FILE*		maskingFile;	//     file to write masked intervals to (can be NULL)
	char*		maskingFilename;//     name of masked intervals file (can be NULL)
	...
	int			needTrueLengths;//     true => we need seq->trueLen to be
								//             .. correct (because we have an
								//             .. anchors file, or because the
...
void start_one_strand     (seq* target, postable* targPositions, seq* query,
                           int emptyAnchors,
                           hitprocessor hitProc, void* hitProcInfo);
