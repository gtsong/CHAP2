...
#define dbg_timing_report_big_stat(field,name) fprintf(stderr,"%-26s %jd\n",name":",seedSearchTimingStats.field)
#endif // dbgTiming
...
static int   filter_seed_hit       (hitprocinfo* hp,
...
static u64 find_table_matches
   (u32		packed2,
	...
	for (pos=pt->last[packed2] ; pos!=noPreviousPos ; pos=pt->prev[pos])
		...
		if (selfCompare)
			{
			if (sameStrand) { if (pos1 >= pos2)               continue; }
			           else { if (pos1 >= (seq2->len-1)-pos2) continue; }
			}
		...
		if (seed_search_dbgDumpRawHits)
			...
			if (seed_search_dbgShowRawHits)
				{
				fprintf      (stderr, "raw seed hit " unsposSlashFmt " ", pos1-len1, pos2-len1);
				print_prefix (stderr, (char*) seq1->v + pos1-seedLength, (int) seedLength);
				fprintf      (stderr, "/");
				print_prefix (stderr, (char*) seq2->v + pos2-seedLength, (int) seedLength);
				fprintf      (stderr, "\n");
				}
			else
				{
...
u64 process_for_plain_hit
   (void*	_info,
	...
	if ((info->hp.minMatches >= 0)
	 && (filter_seed_hit (&info->hp, pos1, pos2, length)))
		return 0;
...
u64 process_for_simple_hit
   (void*	_info,
	...
	if ((info->hp.gfExtend != gfexNoExtend) && (seed_search_dbgShowHits))
		printf ("simple seed hit " unsposSlashFmt " (diag " sgnposFmt ")\n",
		        pos1-(length-1), pos2-(length-1), diagNumber (pos1, pos2));
...
fresh_hit:
	...
	if ((info->hp.gfExtend != gfexNoExtend) && (seed_search_dbgShowHits))
		printf ("simple seed hit " unsposSlashFmt " (diag " sgnposFmt ")\n",
		        pos1-(length-1), pos2-(length-1), diag);
...
//		See note 5.
//
// (3)	$$$ We assume (incorrectly) that the entirety of sequence 1 and 2 are
//		fair game for the HSP.  However, we should limit it to the subsequence
//		passed to seed_hit_search (the start and end arguments).
//
...
static void warn_for_search_limit
   (void)
	...
	char* name2   = (seq2->useFullNames)? seq2->header : seq2->shortHeader;
	char  strand2 = ((seq2->revCompFlags & rcf_rev) == 0)? '+' : '-';

	fprintf (stderr, "WARNING. Query \"%s\", %c strand, contains more than %s HSPs.\n",
					 name2, strand2, commatize(searchLimit));

	if (firstReport)
		{
		fprintf (stderr, "Additional HSPs for this query/strand are treated as if they did not exist.\n");
		firstReport = false;
		}
	}
