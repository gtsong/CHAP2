...
int getc_or_die
   (FILE*	f,
	...
	if (ferror (f))
		...
		suicidef_with_perror ("I/O failure for %s (getc returned %d)",
		                      fileName, errno);
		}
...
//
// Notes:
//	- zalloc_or_die is a malloc() replacement that fills the block of memory
//	  with zeros.
//
//----------
...
//
// [[ see also strncpy_reverse ]]
//
//----------
//
// Arguments:
//	char*	s:		The string to copy.
//	int		len:	Its length (not including the terminating zero).
//
// Returns:
...
char* copy_reverse_of_string
   (char* s,
	int   len)
	{
	char* r = malloc_or_die ("copy_reverse_of_string", len+1);
	char* t;
...
//
// [[ see also copy_reverse_of_string ]]
//
//----------
//
// Arguments:
...
char* unitize
   (const int64	v)
	...
	unit = 0;
	for (rep=vv ; vv>1023 ; vv/=1024,rep/=1024)
		unit++;

	if (rep > 99) { rep /= 1024;  unit++; }
