...
//
//	(2)	For partitioned sequences, p[0].sepPos is always 0 and p[len].sepPos is
//		always seq->len.  seq->v[p[ix].sepPos] is a NUL, for all ix (even for 0
//		and len).  All partitions are bounded on both sides by a NUL, even the
//		first partition.
//
//----------
...
typedef struct partition
	{
	unspos	sepPos;				//     the position (in seq->v) of the
...
typedef struct seqpartition
	...
	// several actual sequences, separated by a NUL character (a zero).
	
	int		state;				//     the current initialization state;  one
								//     .. of seqpart_xxx below

	u32		size;				//     the number of entries allocated for
								//     .. header[], contig[], and sepPos[];
								//     .. note that this must always be at
								//     .. least 1 more than len
...
enum							// states for a partitioned sequence (only valid
	...
	seqpart_empty = 0,			// created by no partitions loaded yet
...
typedef struct seq
	...
	unspos	start;				//     1-based starting location;  this is only
	...
	FILE*	f;					// (F) the file associated with this sequence;
								//     .. this can be NULL (for which fileType
								//     .. must be seq_type_nofile)
	...
	int		nameParseType;		//     how to parse sequecne headers, it at all;
...
#ifdef sequences_owner
char* seqTypeNames[] = {"(no file)", "unknown",
                       "fasta", "csfasta", "nib", "2bit", "hsx", "qdna" };
#else
...
enum
	...
	name_parse_type_alnum,		// trim path on left, use only alphanumeric and
								// .. underscore
	name_parse_type_darkspace,	// trim path on left, use anything but
								// .. whitespace
	name_parse_type_trigger		// use nameTrigger
	};
