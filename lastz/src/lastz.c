...
static const control defaultParams =
	...
	0,									// maxWordCountChasm
	0,									// dynamicMasking
	NULL,NULL,							// maskingFile, maskingFilename
	false,								// reportCensus
	...
	if (lzParams.capsule != NULL)
		...
		if (haveSeveralTargets)
			...
			if (currParams->selfCompare)
				...
				// $$$ --self would require us to read the file separately
				// $$$ .. as both target and query, rather than just use the
				// $$$ .. same sequence object for both;  also, we would have
				// $$$ .. to handle seqA vs seqA as we do now for --self, but
				// $$$ .. would have to handle seqA vs seqB as a normal
...
next_target:
		...
		if (haveSeveralTargets)
			if ((dbgTargetProgress != 0)
			 && ((dbgTargetProgress == 1) || (numTargets % dbgTargetProgress == 1)))
				...
				fprintf (stderr, "(%.3fs) ", ((float)(dbgTargetProgressClock))/clocksPerSec);
				dbgTargetProgressClock = -((s64) read_clock());

				fprintf (stderr, "processing target %d", numTargets);
				if (target->shortHeader != NULL)
	...
	if (target->partition.p != NULL)
		{
		if (currParams->selfCompare)
			suicide ("multiple action cannot be used with --self");
		if (currParams->maskingFilename != NULL)
			suicide ("multiple action cannot be used with --outputmasking");
		}
	...
	if (currParams->maskingFilename != NULL)
		...
		pmi.f   = currParams->maskingFile;
		pmi.seq = target;
		report_census_intervals (targCensus, print_masking_interval, &pmi);
		}
...
void start_one_strand
   (seq*			target,
	...
	//if      (hitProc == process_for_plain_hit)       fprintf (stderr, "hitProc is process_for_plain_hit\n");
	//else if (hitProc == process_for_recoverable_hit) fprintf (stderr, "hitProc is process_for_recoverable_hit\n");
	//else if (hitProc == process_for_simple_hit)      fprintf (stderr, "hitProc is process_for_simple_hit\n");
	//else if (hitProc == process_for_twin_hit)        fprintf (stderr, "hitProc is process_for_twin_hit\n");
	//else                                             fprintf (stderr, "hitProc is (unknown)\n");
...
//ÉÉÉ need to check if the use of the scoring matrix in here is copacetic with inference
...
void finish_one_strand
   (seq*			target,
	...
	// $$$ .. which is not what the user should expect
...
static u32 report_hsps
   (arg_dont_complain(void* info),
	...
	// $$$Êwe should valide that the ends are symmetric about the diagonal
...
//
// mirror_alignments--
//	Reflect gapped alignments across the main diagonal (of DP space).
//
// We "expect" that the two sequences are identical, but in actuality all we
// require is that they have the same length.
//
//----------
//
// Arguments:
...
static alignel* mirror_alignments
   (alignel*	alignList)
	...
	unspos		pos1, end1, pos2, end2, pos2OnPlus, end2OnPlus, x, y;
	...
	for (a=alignList ; a!=NULL ; a=aNext)
		...
		if (sameStrand)
			...
			// not it crosses the diagonal;  if it is below the diagonal we
			// discard it;  if it crosses the diagonal with trucate it before
			// duplication

			pos2OnPlus = seqLen - pos2;
			end2OnPlus = seqLen - end2;	// nota bene: end2OnPlus < pos2OnPlus
			...
			if (end1 >= end2OnPlus)
				...
				//fprintf (stderr, "checking alignment from " unsposCommaFmt " to " unsposCommaFmt ")\n",
				//				 pos1, end1, pos2OnPlus, end2OnPlus);
				...
				if (isTruncated)
					{
					if (x == seqposInfinity) goto discard_alignment;
					...
					if ((x < y) || (x > y+1))
						...
						fprintf (stderr, "WARNING.  Internal error in mirror_alignments().\n"
						                 ...
						                 pos1, end1, pos2OnPlus, end2OnPlus, x, y);
						dontMirror = true;
						}

					a->end1 = end1 = x;
					a->end2 = end2 = seqLen - y;
					end2OnPlus = y;
				...
				a->s = score_alignment (currParams->scoring,
				                        _seq1, pos1, _seq2, pos2, a->script);
				//fprintf (stderr, "rescoring alignment from " unsposCommaFmt " to " unsposCommaFmt ")\n"
				//				 "  new score is " scoreFmt "\n",
				//				 pos1, end1, pos2OnPlus, end2OnPlus, a->s);
			...
			b = malloc_or_die ("mirror_alignments", sizeof(alignel));
			b->isTrivial = false;
			b->beg1 = (seqLen-end2) + 1;  b->end1 = (seqLen-pos2);
			b->beg2 = (seqLen-end1) + 1;  b->end2 = (seqLen-pos1);
			b->s    = a->s;
			b->next = NULL;
...
static void parse_options_loop
   (int			argc,
	...
	while (argc > 0)
		...
		if ((strcmp_prefix (arg, "--ambig=n,")     == 0)
		...
		if ((strcmp_prefix (arg, "--ambig=iupac,")     == 0)
		...
		// --queryhsplimit=<n>
		// this differs from --limitperquery by not setting hspToGapped
		...
		if ((strcmp_prefix (arg, "--traceback=") == 0)
		...
		if (strcmp_prefix (arg, "--debug=targetprogress=") == 0)
			{
			dbgTargetProgress = string_to_int (strchr(argStr,'=')+1);
			goto next_arg;
			}
#endif // allowSeveralTargets
	...
	*lzParams = defaultParams;
	...
	if (lzParams->selfCompare)
		...
		if (lzParams->mirrorHSP == -1)
			{ // these will be changed later in this routine if gappedExtend is
			  // .. true;  the reason we delay is that, at this point, the value
			  // .. of gappdExtend may not be fully established
...
//ÉÉ somewhere I need to check for a mismatch between strandedness and the
//ÉÉ complementabilty of the score set (for quantum query)
