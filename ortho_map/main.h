...
#define TD_PID 50 // in ortho-content, TD_PID is ignored

#define NUM_OVERLAP 1 // the number of overlaps that allow when nothing exists for inferring the next dup
...
#define CONTENT_ORTHO 4 // orthologs mappings based on sequence contents 
#define POSITION_ORTHO 5 // orthologs mappings based on origin positions

#define ALT_EFFEC_VALUE 0 // for strong local alignments
// #define ALT_EFFEC_VALUE 300 // for loose local alignments - they may be unmasked repeats
#define PID_DIFF 5 // threshold value of similarity difference allowed in the second round check
//#define PID_DIFF_TH_STRICT 2// the threshold value of allowing difference of two alignments in latest_dup()
#define PID_DIFF_TH_STRICT 0// the threshold value of allowing difference of two alignments in latest_dup()

#define EFFEC_DEL_LEN_TH 5000 // threshold value used in pred_del.c
#define MIN_GAP 100
...
#define NUM_SP 50 // the number of species including an out-group species
...
#define LEN_NAME 5000 // the length of a gene name
...
#define OLD_DUP 40 // self-alignment involved in "old-dup" conversion 
#define NORMAL 41 // self-alignment unrelated to "old-dup" conversion
#define USED 42 // "old-dup" conversion 
...
#define DEL_TH 30 // Threshold in the decision of the occrrence of deletion
// #define DEL_TH 50 // Threshold in the decision of the occrrence of deletion
...
#define RD_MODE 4
#define UC_MODE 5 // unchaining alignments
#define O_MODE 6 // orthologous alignments
#define S_MODE 7 // read inter-speices alignments from merged one 
#define PAIR_MODE 8 // alignments of two different sequences
...
struct ops_list{
	char sign; // +/-: dup, d/l: del, c/v: con, s: spe, i: inv, n: nothing
	...
	int ctg_id1, ctg_id2; //contig id
	int dir; // direction for conversion and the cutpoint for deletion in present sequence
};
...
struct p_tree{
  ...
  int d_mode; // if this region is an internal node, d_mode is one of three modes - DP, CV, and SP 
	int od; // orientation for printing orthologous alignments
  int sp_code; // a code number of self-alignment: species id, seq id for orthologous mappings
  int gid; // a gene identifier 
	int nid; // a node identifier
  int val;
	int *ch_sp; // the list of children nodes(species for a species tree)
...
struct gap_list{
	int gid;
	int type; 
	/* for gap handling and dup inferring
	...
	*/
	/* in id_list for tree construction
		0: + strand
		1: - strand
	*/
	int id1;
...
struct DotList{
  ...
  int sign; // if the sign is 0, the orientation is the same
	...
	int l_id; // a linked local alignment split by copy into the own region
            // default is -1
						// after the step of redoing duplications and conversions for many-to-many orthologs, it is used to indicate the original index for split initial alignments
	int l_pid;
	int lock; // Use 1 : a parameter of the locking status, default is -1
	          // if the lock is LEFT_LOCK, a left end can not be merged
						...
						// Use 2 : this is used to mark orthologous alignments when generating orthologous mappings. A value is either NON_ORTHO or ORTHO.
						// Use 3 for initial self-alignments : mark OLD_DUP, USED, or -1
...
struct cv_list{
	int fid; // id in the initial list
	int s1, s2, t1, t2; // coordinates of the alignment involved in the conversion ; the interval is closed
	int a1, a2, b1, b2; // coordinates of the source and target region for conversion
	char ori; // the orientation of the alignment
	float algn_pid;
	float conv_pid;
	int dir; // direction of conversion. 0 : unknown; 1 : first sequence is converted; 2 : second sequence is converted
	int stat; // status of orthologs in outgroup. 0: no orthologs; 1 : triplet test; there is only orthologs for paralogs 1; 2 : triplet test; there is only orthologs for paralogs 2; 3 : quadruplet test; 4 : cases for conversion events cover the whole duplicated regions. 
	int sp_id; // 0 for the first species, 1 for the second species
	int sp_code, out_code;
	char ctg_id1, ctg_id2;
};

struct exons_list{
	int fid; // id in the inital list
	struct I reg, cmp_reg;	
	int sp_id;
	int val;
	char sign; // '<' or '>'
	char name[LEN_NAME];
