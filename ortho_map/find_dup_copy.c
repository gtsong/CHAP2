...
bool check_whole_del_region_inclusion(struct DotList *dots, int num_lines, struct I del_reg, int id1, int id2)
{
	int i = 0;
	bool res = false;
	int pid = 0;

	if( dots[id1].identity < dots[id2].identity ) {
		pid = dots[id2].identity;
	}
	else pid = dots[id1].identity;
	
	for( i = 0; i < num_lines; i++ )
	{
		if( (i == id1) || (i == id2) ) {}
		else if( dots[i].identity < (pid - PID_DIFF)) {} // possibility of chaining or aligning errors 
		else if( (subset(del_reg, dots[i].x) == true) || (subset(del_reg, dots[i].y) == true ) ) res = true;
	}
