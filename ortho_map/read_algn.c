...
int count_ncol_rev(struct I reg1, struct I reg2, char *seq, int end_loc, int *beg)
{
	...
	if( reg1.lower >= reg2.lower ) ncol = end_loc;
	else {
		k = end_loc;
		count = 0;
		while( (k >= 0) && (count < abs(reg1.lower - reg2.lower)) ) {
			while( (k >= 0) && (seq[k] == '-') ) k--;
			count++;
			k--;
		}
  ...
  if( reg1.upper <= reg2.upper ) *beg = 0;
	else {
		k = 0;
		count = 0;
		while( (k < end_loc) && (count < abs(reg1.upper - reg2.upper)) ) {
			while( (k < end_loc) && (seq[k] == '-') ) k++;
			count++;
			k++;
		}
...
{ 
	int beg = 0, len = 0, end = 0, end_pos = 0;
	struct b_list *a_info;
	int i = 0;
	int count = 0;
	...
	if( sp != SELF1 ) {
		fatalf("counting available based on only first sequence %d\n", sp);
	}

	beg = algns[id].xl_diff + algns[id].xl_offset;
  len = algns[id].x.upper - algns[id].xr_diff - algns[id].xr_offset - algns[id].x.lower - algns[id].xl_offset;
  end = find_xloc_one(algns[id], f, len, NO_GAP_INC);

  get_nth_algn(S1, T1, algns[id].fid, beg, f, a_info, REG);
	free(a_info);
  ...
  if( S1[strlen(S1)-1] == '\n' ) end_pos = strlen(S1)-1;
	...
	i = 0;
	while( (i < BIG) && (i < end) && (S1[i] != '\0') && (S1[i] != '\n')) {
		if( strchr("N", toupper(S1[i])) ) count++;
		i++;
	}
...
int countN_in_seq(char *seq, int b, int e)
{
  ...
  i = b;
  while( (i < BIG) && (i < e) && (seq[i] != '\0') && (seq[i] != '\n')) {
    if( strchr("N", toupper(seq[i])) ) count++;
    i++;
  }
