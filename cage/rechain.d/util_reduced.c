...
void sort_by_bid(struct slist *st, struct DotList *dots, int num)
{
	int i, j;
	int cur_val;
	...
	while(i < num)
	{
		...
		{
			st[i+j].val = dots[st[i+j].id].x.lower; // This part could be changed
			j++;
		}
...
void sort_exons(struct exons_list *a, int num)
{
  ...
  quick_sort_inc_exons(a, 0, num-1, POS_BASE);  
	...
	{    
		j = 0;    
		while(((i+j) < num) && (a[i].reg.lower == a[i+j].reg.lower)) j++;
...
{	
	...
	{    
		if( mode == POS_BASE ) {
			while (a[i].reg.lower<x) i++; 
			while (a[j].reg.lower>x) j--;
		}
		else if( mode == LEN_BASE ) {
			while (width(a[i].reg)<x) i++; 
			while (width(a[j].reg)>x) j--;
		}

		if (i<=j)
		{
			h.reg = assign_I(a[i].reg.lower, a[i].reg.upper);
			h.val = a[i].val;
			a[i].reg = assign_I(a[j].reg.lower, a[j].reg.upper);
			a[i].val = a[j].val;
			a[j].reg = assign_I(h.reg.lower, h.reg.upper);
			a[j].val = h.val;
			i++; 
			j--;
		}
