  ...
  if( debug_mode == TRUE ) printf("Interval in the original dot plot: [%d, %d]\n", b, e);
	...
	if( (mode == SECOND_RUN) && (is_tandem(algns[id]) == false)) {
		if( (b != -1) && (e != -1) && (src_b != -1) && (src_e != -1) ) {
			...
			temp_ops[0].pid = pid;
			temp_ops[0].pid = pid;
			temp_ops[0].sp_id = init_algns[org_id].sp_id;
...
*/
      	...
      	fprintf(output_f, "s %s %d %d + %d %s", sp1_name, (a_info->b1)-1, a_info->e1, a_info->len1, S2);
      	len = strlen(S2);
      	if( S2[len-1] != '\n' ) fprintf(output_f, "\n");
      	...
      	fprintf(output_f, "s %s %d %d %c %d %s", sp2_name, a_info->b2, a_info->e2, a_info->strand, a_info->len2, T2);
      	len = strlen(T2);
      	if( T2[len-1] != '\n' ) fprintf(output_f, "\n");
      	fprintf(output_f, "\n");
			}
...
void write_init_maf_stdout(struct DotList *init_algns, int num_algns, struct n_pair *contigs1, struct n_pair *contigs2, int *len_sum1, int *len_sum2, int num1, int num2, int len1, int len2, FILE *fp, int flag)
{
  ...
  a_info->pid = 0;
	...
	if( num1 > 0 ) {
		contigs_len1 = (int *) ckalloc(sizeof(int) * num1);
		for( i = 0; i < num1; i++ ) {
			contigs_len1[i] = 0;
			if( i == (num1-1) ) {
				contigs_len1[i] = len1 - len_sum1[i];
			}
			else {
				contigs_len1[i] = len_sum1[i+1] - len_sum1[i];
			}
		}
	}
	else if( num1 == 0) {
		contigs_len1 = (int *) ckalloc(sizeof(int));
		contigs_len1[0] = len1;
	}
	else {
	...
	if( num2 > 0 ) {
		contigs_len2 = (int *) ckalloc(sizeof(int) * num2);
		for( i = 0; i < num2; i++ ) {
			contigs_len2[i] = 0;
			if( i == (num2-1) ) {
				contigs_len2[i] = len2 - len_sum2[i];
			}
			else {
				contigs_len2[i] = len_sum2[i+1] - len_sum2[i];
			}
		}
	}
	else if( num2 == 0) {
		contigs_len2 = (int *) ckalloc(sizeof(int));
		contigs_len2[0] = len2;
	}
	else {
		fatalf("illegal negative number: %d\n", num1);
  ...
  printf("##maf version=1 scoring=lastz-pid\n");
	...
	for( i = 0; i < num_algns; i++ ) 
	{
		if( (init_algns[i].sp_id == flag) && (init_algns[i].sign != DELETED)) 
		{
			...
			x_b = init_algns[i].x.lower + init_algns[i].xl_diff - 1;
			(*a_info).b1 = x_b;
			y_b = init_algns[i].y.lower + init_algns[i].yl_diff;
			y_e = init_algns[i].y.upper - init_algns[i].yr_diff;
			...
			else (*a_info).len1 = contigs_len1[ctg_id1];
			...
			else (*a_info).len2 = contigs_len2[ctg_id2];
...
/*
			if( ctg_id1 != -1 ) {
				(*a_info).b1 = x_b - len_sum1[ctg_id1];
			}
			else (*a_info).b1 = x_b;
			...
			if( init_algns[i].sign == 0 ) {
				(*a_info).strand = '+';
				if( ctg_id2 != -1 ) {
					(*a_info).b2 = y_b-1-len_sum2[ctg_id2];
				}
				else {
					(*a_info).b2 = y_b-1;
				}
				...
				if( ctg_id2 != -1 ) {
					(*a_info).b2 = (a_info->len2) - y_e + 1 + len_sum2[ctg_id2];
				}
				else {
					(*a_info).b2 = (a_info->len2) - y_e + 1;
				}
...
*/
			...
			if( (e1 >= MIN_GAP) && (e2 >= MIN_GAP) ) {
   	 	  ...
   	 	  pid = (int)(cal_pid_maf(S2, T2, end) + 0.5);
				...
				if( (((a_info->b1-1)+(a_info->e1)) > (a_info->len1)) || ((a_info->b1-1) < 0) ) {
					fatalf("line 1265: Bad coordinates %d %d len %d\n", a_info->b1-1, a_info->e1, a_info->len1);
				}
      	...
      	if( S2[len-1] != '\n' ) printf("\n");
				...
				if( (((a_info->len2) - (a_info->b2-1) - (a_info->e2)) < 0) || ((a_info->b2-1) < 0) ) {
					fatalf("line 1274: Bad coordinates %d %d len %d\n", a_info->b2-1, a_info->e2, a_info->len2);
				}
