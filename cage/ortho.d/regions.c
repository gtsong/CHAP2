...
bool f_loose_subset(struct I reg1, struct I reg2, int th)
{
	struct I temp;
	int threshold;
...
bool loose_subset(struct I reg1, struct I reg2)
{
	struct I temp;
...
bool too_loose_subset(struct I reg1, struct I reg2)
{
	struct I temp;
  ...
  int i = 0;
	...
	for(i = 0 ; i < *num; i++)
	{
		if( dots[i].l_id != -1 )
		{
			...
			dots[dots[i].l_id].init_sign = dots[i].init_sign;
			dots[i].x = dots[i].m_x;
			dots[i].y = dots[i].m_y;
			dots[i].l_id = -1;
    ...
    if( (dots[i].sign == 0) || (dots[i].sign == 1) || (dots[i].sign == ORTHO) || (dots[i].sign == ORTHO_COMP) )
    {
      ...
      dots[j].y = assign_I(dots[i].y.lower, dots[i].y.upper);
	    ...
	    dots[j].identity = dots[i].identity;
			...
			dots[j].indiv_fid = dots[i].indiv_fid;
			dots[j].xl_offset = dots[i].xl_offset;
			dots[j].xr_offset = dots[i].xr_offset;
			dots[j].yl_offset = dots[i].yl_offset;
			dots[j].yr_offset = dots[i].yr_offset;
			dots[j].len1 = dots[i].len1;
...
bool both_ends_overlap(struct I reg1, struct I reg2, int th)
{
	struct I tmp1, tmp2;
	int threshold = 0;
	int cur_th = 0;
	...
	tmp1 = assign_I(reg1.lower - (2*threshold), reg1.lower + (2*threshold));
	tmp2 = assign_I(reg1.upper - (2*threshold), reg1.upper + (2*threshold));
	if( (in(reg2.lower, tmp1) == false) && (in(reg2.upper, tmp2) == false) ) {
		return(true);
	}
	else {
		return(false);
