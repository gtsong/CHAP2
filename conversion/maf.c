  ...
  while ( *ptr1 != '\0') {
    ...
    if ( strncmp(buffer, "score=", 6) == 0 ) {
      ali->score = atof(buffer+6);
		...
		else if ( strncmp(buffer, "srcblock=", 9) == 0 ) {
			ali->score = atof(buffer+9);
		}
...
void mafWrite(FILE *f, struct mafAli *a) {
	...
	for (row = 0, c = a->components; c != NULL; c = c->next, row++) {
	  switch ( c->paralog ) {
	  case 's': break;
	  case 'a': fprintf(f, " amplifier=%d", row);
	    break;
	  case 'c': fprintf(f, " copy=%d", row);
	    break;
	  default: fatalf("Wrong character: \'%c\'", c->paralog);
	  }
	}	  
	fprintf(f, "\n");
